...
class ExperienceCollector:
    def __init__(self, board_size: int, num_planes: int):
        """[summary]
    ...
    def complete_episode(self, reward: float) -> None:
        self.states, self.visit_counts = self.episode_augmentation()
        self.rewards = np.full(len(self.states), reward)
    ...
    def episode_augmentation(self) -> Tuple[AbstractGameState, Iterable[np.ndarray]]:
        # rotate
        new_states = np.zeros((len(self._current_episode_states) * 4, self.num_planes, self.board_size, self.board_size))
        new_visit_counts = np.zeros((len(self._current_episode_states) * 4, self.board_size * self.board_size))
        idx = 0
        for state, visit_count in zip(self._current_episode_states, self._current_episode_visit_counts):
            for i in range(4):
                ...
                new_states[idx] = rotated_state
                new_visit_counts[idx] = rotated_visit_count.flatten()

                new_states[idx] = np.flip(rotated_state, 2)
                new_visit_counts[idx] = np.fliplr(rotated_visit_count).flatten()
                idx += 1
