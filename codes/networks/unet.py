...
class DoubleConvEmbeddingBlock(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        super(DoubleConvEmbeddingBlock, self).__init__()
        self.layers = nn.Sequential(
...
class DoubleConvEmbeddingSkipBlock(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        super(DoubleConvEmbeddingSkipBlock, self).__init__()
        self.layers = nn.Sequential(
            DoubleConvBlock(in_channels, in_channels, residual=True),
    ...
    def forward(self, x, x_skip, embedding):
        emb = self.emb_layer(embedding)
        emb = emb[:, :, None, None].repeat(1, 1, x.shape[-2], x.shape[-1])
...
class SimpleUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, noise_steps=1000, time_dim=256, state_dim=256):
        super(SimpleUNet, self).__init__()
        
        self.emb_dim = time_dim + state_dim + 1  # player tensor(0 or 1)
        
        self.pos_encoding = PositionalEncoding(embedding_dim=time_dim, max_len=noise_steps)

        self.conv1 = DoubleConvEmbeddingBlock(in_channels, 32, emb_dim=self.emb_dim)
        self.self_attention1 = SelfAttentionBlock(32, 3)
        self.conv2 = DoubleConvEmbeddingBlock(32, 64, emb_dim=self.emb_dim)
        self.self_attention2 = SelfAttentionBlock(64, 3)

        self.conv3 = DoubleConvBlock(64, 128)
        self.conv4 = DoubleConvBlock(128, 128)
        self.conv5 = DoubleConvBlock(128, 64)

        self.conv6 = DoubleConvEmbeddingSkipBlock(128, 32, emb_dim=self.emb_dim)
        self.self_attention3 = SelfAttentionBlock(32, 3)
        self.conv7 = DoubleConvEmbeddingSkipBlock(64, 16, emb_dim=self.emb_dim)
        self.self_attention4 = SelfAttentionBlock(16, 3)
        
        self.out_layer = nn.Conv2d(16, out_channels, kernel_size=1)
        
    def forward(self, x, t, z, cur_p):
        """

        Args:
            x (_type_): _description_
            t (_type_): _description_
            s (_type_): game board를 vae를 사용하여 mu와 log_var로 바꾼 것
        ...
        x = self.conv6(x3, x2, emb)
        x = self.self_attention3(x)
        x = self.conv7(x, x1, emb)
